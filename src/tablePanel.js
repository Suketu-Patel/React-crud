...
class TablePanel extends React.Component {
    ...
    render() {
        //maps state.data to the table, to display.
        const table = this.props.state.data.map((item) => {
            return (
                <tr key={item.email}>
                    ...
                    <td>{item.gender}</td>
                    <td>
                        <button className="actions" onClick={(e) => this.props.handleEdit(e, item.email)}>
                            <i className="fas fa-edit"></i>
                        </button>
                        <button className="actions" onClick={(e) => this.props.handleRemove(e, item.email)}>
